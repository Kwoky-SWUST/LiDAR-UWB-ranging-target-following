...
void uwbProcess(const uwb_lidar_tracking::UltraWBAoa::ConstPtr &uwb_msg)
{
    ...
    for (const auto &node : uwb_msg->nodes)
    {
        ...
        if (m_uwbs.find(node_id) != m_uwbs.end())
        {
            ...
            existing_node.current_measure.angle = node.angle;
            existing_node.current_measure.x = node.dis * cos(degreesToRadians(node.angle));
            existing_node.current_measure.y = node.dis * sin(degreesToRadians(node.angle));
            existing_node.trajectory_raw.push_back(Point(existing_node.current_measure.x, existing_node.current_measure.y));
        ...
        else
        {
            ...
            new_node.current_measure.angle = node.angle;
            new_node.current_measure.x = node.dis * cos(degreesToRadians(node.angle));
            new_node.current_measure.y = node.dis * sin(degreesToRadians(node.angle));
            new_node.trajectory_raw.push_back(Point(new_node.current_measure.x, new_node.current_measure.y));
...
void associateClusters(std::map<int, Cluster> &current_clusters, const ros::Time &current_time)
{
    ...
    for (auto &t_cluster : current_clusters)
    {
        if(!t_cluster.second.status)
        {
            ...
            {
                std::cout << "Retracking Successful!" << std::endl;
...
double getGaussianSimilarity2(const Object &input_obj, const UltraWB &input_uwb)
{
    // we should only consider the valid lidar points when calculating similarity
    if (input_obj.trajectory.size() < 5 || input_uwb.trajectory_raw.size() < 5)
    {
        if (input_obj.association_count < 5)
        {
            return 0.0;
    ...
    int valid_size = std::min(input_obj.trajectory.size(), input_uwb.trajectory_raw.size());

    double distance_sum = 0, distance_avg = 0, angle_sum = 0, angle_avg = 0, count = 0, sigma_r = 1.0, sigma_a = 1.0;

    // we calculate the similarity based on range and angle respectively,
    // so that we can give different weight to them, cuz the accuracy of ranging and angle is not the same

    auto it_uwb_s = input_uwb.trajectory_raw.end();
    auto it_obj_s = input_obj.trajectory.end();
    // double current_distance = euclideanDistance(*it_uwb_s, *it_obj_s);
    // double current_similarity = std::exp(-0.5 * current_distance / std::pow(sigma, 2));

    for (int i = 1; i <= valid_size; ++i)
    {

        Point uwb_point = *(it_uwb_s - i);
        Point obj_point = *(it_obj_s - i);

        // we only consider the valid point
        if (obj_point.status)
        {
            double range_1 = sqrt((uwb_point.x * uwb_point.x) + (uwb_point.y * uwb_point.y));
            double range_2 = sqrt((obj_point.x * obj_point.x) + (obj_point.y * obj_point.y));
            distance_sum += std::fabs(range_1 - range_2);

            double angle_1 = atan2(uwb_point.y, uwb_point.x);
            double angle_2 = atan2(obj_point.y, obj_point.x);
            angle_sum += std::fabs(angle_1 - angle_2);
    ...
    double comprehensive_similarity = std::exp(-0.5 * distance_avg / std::pow(sigma_r, 2)) * std::exp(-0.5 * angle_avg / std::pow(sigma_a, 2));
    //return current_similarity * std::exp(-0.5 * distance_avg / std::pow(sigma, 2));
...
void sequenceLocalization(ros::Time current_time)
{
    ...
    for (auto &uwb_node : m_uwbs)
    {
        ...
        double max_similarity2 = -std::numeric_limits<double>::infinity();

        int matched_obj_index = -1;
        int matched_obj_index2 = -1;
        for (auto &t_object : m_objects)
        {
            if (t_object.second.status && !t_object.second.is_occluded)
            {
                double g_similarity = getGaussianSimilarity(t_object.second, uwb_node.second);
                double g_similarity2 = getGaussianSimilarity2(t_object.second, uwb_node.second);
                if (g_similarity > max_similarity)
                {
                    max_similarity = g_similarity;
                    matched_obj_index = t_object.second.id;
                }
                if (g_similarity2 > max_similarity2)
                {
                    max_similarity2 = g_similarity2;
                    matched_obj_index2 = t_object.second.id;
                }
            }
        }
        if (max_similarity2 > similarity_threshold && matched_obj_index2 > 0 && m_objects[matched_obj_index2].width < 2)
        {
            m_objects[matched_obj_index2].probability_count = m_objects[matched_obj_index2].probability_count + 1.0;
            double proba_count = m_objects[matched_obj_index2].probability_count / (m_objects[matched_obj_index2].association_count - trajectory_length_threshold + 1.0);
            t_target.probability = std::min(proba_count, 1.0);
            // prevent from localization result jumping

            if (t_target.probability > 0.5) //&& euclideanDistance(m_objects[matched_obj_index].trajectory.back(), uwb_node.second.trajectory.back()) < nn_threshold
            {
                t_target.id = matched_obj_index2;
                t_target.uwb_id = uwb_node.second.id;
                t_target.status = true;
                t_target.x = m_objects[matched_obj_index2].trajectory.back().x;
                t_target.y = m_objects[matched_obj_index2].trajectory.back().y;
                t_target.is_occluded = false;
                uwb_node.second.matched_object_id = matched_obj_index2;
                uwb_node.second.trajectory.push_back(m_objects[matched_obj_index2].trajectory.back());

                m_objects[matched_obj_index2].matched_uwb_id = uwb_node.second.id;
                t_targets.targets.push_back(t_target);
                plotTarget(m_objects[matched_obj_index2]);

                // for long-term run, re-set the probability to 1;
                if(m_objects[matched_obj_index2].probability_count > trajectory_length)
                {
                    m_objects[matched_obj_index2].association_count = 0.25 * trajectory_length + trajectory_length_threshold;
                    m_objects[matched_obj_index2].probability_count = 0.25 * trajectory_length;
                }
            }
            else
            {
                std::cout<< "Filtered wrong lidar localization! Using UWB AoA location!" << std::endl;
                t_target.id = matched_obj_index2;
                t_target.uwb_id = uwb_node.second.id;
        ...
        //plot the aoa trajectory
        plotAoaTrajectory(uwb_node.second);
    }
    targets_pub.publish(t_targets);
    t_targets.targets.clear();
}

void Callback(const sensor_msgs::LaserScan::ConstPtr &scan_msg, const uwb_lidar_tracking::UltraWBAoa::ConstPtr &uwb_msg)
{
...
int main(int argc, char **argv)
{
    ...
    message_filters::Subscriber<uwb_lidar_tracking::UltraWBAoa> range_sub(nh, uwb_topic, 1);
    typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::LaserScan, uwb_lidar_tracking::UltraWBAoa> sync_policy;
