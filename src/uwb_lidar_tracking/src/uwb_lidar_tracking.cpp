...
double getGaussianSimilarity(const Object &input_obj, const UltraWB &input_uwb)
{
    ...
    if (input_obj.trajectory.size() < 20 || input_uwb.s_uwb_meas.size() < 20)
    {
        if(input_obj.association_count < 20)
        {
            return 0.0;
        }
    }
...
void sequenceLocalization(ros::Time current_time)
{
    ...
    for (auto &uwb_node : m_uwbs)
    {
        ...
        for (auto &t_object : m_objects)
        {
            if (t_object.second.status && !t_object.second.is_occluded && t_object.second.width < 2)
            {
        ...
        if (max_similarity > similarity_threshold && matched_obj_index > 0)
        {
            ...
            if (t_target.probability > 0.5) //&& euclideanDistance(m_objects[matched_obj_index].trajectory.back(), uwb_node.second.trajectory.back()) < nn_threshold
            {
                ...
                t_target.status = true; //这个status 是表明目标是否 序列相似度、宽度、可能性 满足要求
