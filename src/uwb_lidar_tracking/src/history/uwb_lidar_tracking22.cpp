/*带特征定位（target localization 函数）的版本，+ 匹配定位 + 重跟踪*/
...
std::vector<Object> objectInit(LidarCluster &input_cluster, std::vector<Object> &objects_updating)
{
    ...
    {   
        for (ObjectIterator it_obj = objects.begin(); it_obj != objects.end(); ++it_obj)
        {
            ...
            {
                //TODO predict
                ros::Time retracking_start_time = ros::Time::now();
                ...
                if(flag_is_svm_model_created)
                {
                    ...
                    for (int i = 0; i < num_particle_samples; i++)
                    {
                        ...
                        if (distance < 0.6 && predict_result > 0)
                        {
                ...
                ros::Time retracking_end_time = ros::Time::now();
                double processing_time = retracking_start_time.toSec() - retracking_end_time.toSec();
                //写文件
                std::ofstream time_consumption_file2(time_consumption_file_name2, std::ios::app);
                time_consumption_file2 << toString(processing_time) << "\n";
                time_consumption_file2.close();
            }
        }
    }
    if (!find_lost_object && !find_overlap_object)
...
void uwbPreprocessing(const nlink_parser::LinktrackNodeframe2::ConstPtr &range_msg)
{
    ...
    if (flag_uwb_debug)
    {
        ...
        {
            std::cout << "id range isupdate lostime historyssize " << it_uwb->id << " " << it_uwb->now_state.range << " " << it_uwb->is_updated << " " << it_uwb->lost_time << " " << it_uwb->history.size() << std::endl;
        }
    }
}
...
void recordTimeConsumption(double &time_consumption1, double &time_consumption2, double &time_consumption3)
{
    //std::cout << "time consumption :" << time_consumption << std::endl;
    std::ofstream time_consumption_file(time_consumption_file_name, std::ios::app);
    time_consumption_file << time_consumption1 << " " << time_consumption2 << " " << time_consumption3 << "\n";
...
void callback(const sensor_msgs::LaserScan::ConstPtr &scan_msg, const nlink_parser::LinktrackNodeframe2::ConstPtr &range_msg)
{
    ...
    else
    {
        ...
        std::cout << "timestamp : " << toString(time_stamp) << std::endl;
        time_counter++;
        ...
        ros::Time dbscan_time_now = ros::Time::now();
        double dbscan_time = dbscan_time_now.toSec() - start_time.toSec();

        // we associate clusters with objects
        clustersToObjects();
        ros::Time associate_time_now = ros::Time::now();
        double associate_time = associate_time_now.toSec() - dbscan_time_now.toSec();
        ...
        sequenceFusion();
        ros::Time catulate_time_now = ros::Time::now();
        double catulate_time = catulate_time_now.toSec() - associate_time_now.toSec();
        ...
        //double time_consumption = end_time.toSec() - start_time.toSec();
        recordTimeConsumption(dbscan_time, associate_time , catulate_time);
    }
