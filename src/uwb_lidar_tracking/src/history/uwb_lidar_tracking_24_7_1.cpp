...
std::string lidar_scan_topic = "/fake_scan_new" ;//"/fake_scan"; //"/robot1/scan";
...
public:
    int id;
    bool status; // means: is associated?
    ...
    void getWidth()
    {
        ...
        this->width = sqrt(pow(this->points.front().x - this->points.back().x, 2) + pow(this->points.front().y - this->points.back().y, 2));
    }
};
...
void associateClusters(std::map<int, Cluster> &current_clusters, const ros::Time &current_time)
{
    ...
    for (auto t_object_it = m_objects.begin(); t_object_it != m_objects.end();)
    {
        ...
        {
            // we should clear the trajectory
            // so that the match process will only focus on the frames later than the occlusion
            t_object_it->second.trajectory.clear();
            //std::cout << "object " << t_object_it->second.id << " is occluded!" << std::endl;

            if (fabs(current_time.toSec() - t_object_it->second.current_updated_time.toSec()) > lost_time_threshold)
            {
                ROS_INFO("Object %d is lost for more than %f seconds, deleted!", t_object_it->second.id, lost_time_threshold);
...
void sequenceLocalization(ros::Time current_time)
{
    ...
    for (auto &uwb_node : m_uwbs)
    {
        ...
        {
            if (m_objects[uwb_node.second.matched_object_id].is_occluded)
            {
        ...
        if (max_similarity > similarity_threshold && matched_obj_index > 0)
        {
            ...
            double proba_count = m_objects[matched_obj_index].probability_count / (m_objects[matched_obj_index].association_count - trajectory_length_threshold);
            t_target.probability = std::min(proba_count, 1.0);
            // prevent from localization result jumping
            std::cout << "Object " << matched_obj_index << " probability_count: " << m_objects[matched_obj_index].probability_count << std::endl;
            std::cout << "Object " << matched_obj_index << " probability: " << proba_count << std::endl;
            std::cout << "Distance from last localization: " << euclideanDistance(m_objects[matched_obj_index].trajectory.back(), uwb_node.second.trajectory.back()) << std::endl;
            ...
            {
                std::cout<< "Filtered wrong localization! " << std::endl;
                t_target.id = matched_obj_index;
                t_target.uwb_id = uwb_node.second.id;
