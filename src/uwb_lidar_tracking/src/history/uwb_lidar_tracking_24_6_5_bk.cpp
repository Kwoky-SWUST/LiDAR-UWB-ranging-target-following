/*标准版 序列匹配+重跟踪 定位 带特征定位 和单帧uwb定位*/
...
std::string amcl_file_name("/home/gglin/WORKSPACE/uwb_lidar_following/dataset/record/amcl_record.txt");
std::string uwb_file_name("/home/gglin/WORKSPACE/uwb_lidar_following/dataset/record/uwb_record.txt");
std::string right_feature_file_name("/home/gglin/WORKSPACE/uwb_lidar_following/dataset/right_feature.txt");
std::string wrong_feature_file_name("/home/gglin/WORKSPACE/uwb_lidar_following/dataset/wrong_feature.txt");
std::string sa_file_name("/home/gglin/WORKSPACE/uwb_lidar_following/dataset/record/searchingarea.txt");
std::string screenshot_file_name("/home/gglin/WORKSPACE/uwb_lidar_following/dataset/record/screenshot.txt");
std::string svdd_localization_file_name("/home/gglin/WORKSPACE/uwb_lidar_following/dataset/record/svdd_localization.txt");
std::string uwb_localization_file_name("/home/gglin/WORKSPACE/uwb_lidar_following/dataset/record/single_uwb_localization.txt");
std::string time_consumption_file_name("/home/gglin/WORKSPACE/uwb_lidar_following/dataset/record/time_consumption_record.txt");
std::string time_consumption_file_name2("/home/gglin/WORKSPACE/uwb_lidar_following/dataset/record/time_consumption_record2.txt");
const char* right_feature_model_name("/home/gglin/WORKSPACE/uwb_lidar_following/dataset/right_feature.model");
const char* svm_model_name("/home/gglin/WORKSPACE/uwb_lidar_following/dataset/right_feature.txt.model");
...
const int       trajectory_length = 200;            //can be considered as the length of the sequence
const double    nearest_neighbor_threshold = 0.5;
const int       lost_time_threshold = 50;          //how long we keep this object if there is no match scan, the unit is frame
const int       trajectory_length_threshold = 20;   //to determine whether this cluster is a noise
const double    identical_distance_threshold = 0.3;
...
int             target_lost_time = 0;
int             misleading_obj_continue_time = 20;
int             time_count_frame = 0;
double          uwb_range_measurement = 0;

double          valid_area_x_min = 0.5;            //Used to exclude clusters outside of this area
double          valid_area_x_max = 12.0;
double          valid_area_y_min = -4;
double          valid_area_y_max = 4.0;

std::string     plot_min_x = "0";                   //plot aera
std::string     plot_max_x = "12";
...
bool lidarPointCompare(LidarPoint p_1,LidarPoint p_2)
{
    return (p_1.angle < p_2.angle);
}
...
bool clusterValidation(LidarCluster &input_cluster)
{
    ...
    // Determine if the num of points is correct
    if (input_cluster.points.size() > 7)
        flag_valid_2 = true;
...
void getClusterFeatures(LidarCluster &input_cluster)
{
    input_cluster.width = sqrt(carmen_square(input_cluster.points.front().x - input_cluster.points.back().x) + carmen_square(input_cluster.points.front().y - input_cluster.points.back().y));

    input_cluster.girth = 0;

    double a, b, c, s, max_d = 0;
    c = input_cluster.width;
    for (std::vector<LidarPoint>::iterator it_point = input_cluster.points.begin() + 1; it_point != input_cluster.points.end(); ++it_point)
    {
        input_cluster.girth += sqrt(carmen_square(it_point->x - (it_point - 1)->x) + carmen_square(it_point->y - (it_point - 1)->y));
        a = sqrt(carmen_square(input_cluster.points.front().x - it_point->x) + carmen_square(input_cluster.points.front().y - it_point->y));
        b = sqrt(carmen_square(input_cluster.points.back().x - it_point->x) + carmen_square(input_cluster.points.back().y - it_point->y));
        s = 0.5 * (a + b + c);
        double d = (2 * sqrt(s * (s - a) * (s - b) * (s - c))) / c;
        if (d > max_d)
            max_d = d;
...
void targetLocalization(double &time_stamp)
{
    for (ClusterIterator it_clu = clusters.end() - 1; it_clu != clusters.begin(); --it_clu)
    {
        getClusterFeatures(*it_clu);
        svm_node *svm_predict_input = new svm_node[feature_num + 1];
        svm_predict_input[0].index = 0;
        svm_predict_input[0].value = it_clu->area;
        svm_predict_input[1].index = 1;
        svm_predict_input[1].value = it_clu->depth;
        svm_predict_input[2].index = 2;
        svm_predict_input[2].value = it_clu->girth;
        svm_predict_input[3].index = 3;
        svm_predict_input[3].value = it_clu->width;
        svm_predict_input[4].index = -1;
        double predict_result = svm_predict(model, svm_predict_input);
        if(predict_result > 0)
        {
            std::ofstream svdd_localization_file(svdd_localization_file_name, std::ios::app);
            svdd_localization_file << std::to_string(time_stamp) << " " << std::to_string(it_clu->center_coordinate.x) 
            << " " << std::to_string(it_clu->center_coordinate.y) << "\n";
            std::cout << "localization success!" << std::endl;
            break;
        }
    }
}

void targetLocalizationUWB(double &time_stamp)
{
    double mindiff = 10000000;
    double location_x,location_y;
    for (ClusterIterator it_clu = clusters.begin(); it_clu != clusters.end(); ++it_clu)
    {
        double distance = sqrt(carmen_square(it_clu->center_coordinate.x) + carmen_square(it_clu->center_coordinate.y)); 
        double diff = fabs(uwb_range_measurement - distance);
        if(diff < mindiff)
        {
            mindiff = diff;
            location_x = it_clu->center_coordinate.x;
            location_y = it_clu->center_coordinate.y;
        }
    }
    if(mindiff < 100)
    {
        std::ofstream uwb_localization_file(uwb_localization_file_name, std::ios::app);
        uwb_localization_file << std::to_string(time_stamp) << " " << std::to_string(location_x) 
        << " " << std::to_string(location_y) << "\n";
        std::cout << "localization success!" << std::endl;
    }
}

void sortClusterPoints()
{
    std::vector<LidarCluster> temp_cluster_set;
    for(ClusterIterator it_cluster = clusters.begin();it_cluster!=clusters.end();++it_cluster)
    {
        LidarCluster temp_cluster = *it_cluster;
        sort(temp_cluster.points.begin(),temp_cluster.points.end(),lidarPointCompare);
        temp_cluster_set.push_back(temp_cluster);
    }
    clusters.clear();
    clusters = temp_cluster_set;
}

int generateRandomID()
{
    int objID = rand() % 1000 + 1;
...
Object objectUpdate(LidarCluster &input_cluster, Object &input_object , int mode)
{
    ...
    if(mode == 1)
    {
        ...
        object_update.getFeatures();
        object_update.getMovingStatus();
        object_update.now_state.is_valid = true;
    }
    if(mode == 2)
    {
        std::cout << "merging two clusters!" << std::endl;
    ...
    {   
        for (ObjectIterator it_obj = objects.begin(); it_obj != objects.end(); ++it_obj)
        {
            ...
            if (it_obj->is_lost && !it_obj->is_updated && it_obj->searching_area_is_created && it_obj->lost_time > 3)
            {
                ...
                if(flag_is_svm_model_created)
                {
                    getClusterFeatures(input_cluster);
                    svm_node* svm_predict_input = new svm_node[feature_num + 1];
                    svm_predict_input[0].index = 0;
                    svm_predict_input[0].value = input_cluster.area;
                    svm_predict_input[1].index = 1;
                    svm_predict_input[1].value = input_cluster.depth;
                    svm_predict_input[2].index = 2;
                    svm_predict_input[2].value = input_cluster.girth;
                    svm_predict_input[3].index = 3;
                    svm_predict_input[3].value = input_cluster.width;
                    svm_predict_input[4].index = -1;
                    double predict_result = svm_predict(model,svm_predict_input);
                    ...
                    for (int i = 0; i < num_particle_samples; i++)
                    {
                        ...
                        //TODO UWB range 
                        double diff = fabs(sqrt(carmen_square(input_cluster.center_coordinate.x) + carmen_square(input_cluster.center_coordinate.y)) - uwb_range_measurement);
                        std:: cout << " UWB range diff = " << diff << " \n \n"<< std::endl;
                        if (distance < 0.5 && diff < 0.2) //  predict_result > 0 && &&    predict_result > 0  0.6  &&   predict_result > 0 
                        {
                            std::cout << "searching distance minimal : " << distance << std::endl;
...
void uwbPreprocessing(const nlink_parser::LinktrackNodeframe2::ConstPtr &range_msg)
{
    ...
    if (!uwb_nodes.empty() && uwb_lost_time < 5)
    {
        ...
        for (UwbNodeIterator it_uwb_node = uwbs.begin(); it_uwb_node != uwbs.end(); ++it_uwb_node)
        {
            if (!it_uwb_node->is_updated && it_uwb_node->lost_time < lost_time_threshold)
            {
                UwbNode uwb_temp;
                uwb_temp = *it_uwb_node;
                uwb_temp.lost_time++;
        ...
        uwb_range_measurement = new_uwbs[0].now_state.range;

        if (flag_plot_uwb_history)
            plotUwbHistory(new_uwbs);
    }
    
    if (uwb_nodes.empty()) // there comes no new uwb message, so we clean vector uwbs after a while
    {
        for (UwbNodeIterator it_uwb_node = uwbs.begin(); it_uwb_node != uwbs.end(); ++it_uwb_node)
        {
    ...
    if (flag_uwb_debug)
    {
        ...
        {
            std::cout << "id range is update lostime historys size " << it_uwb->id << " " << it_uwb->now_state.range << " " << it_uwb->is_updated << " " << it_uwb->lost_time << " " << it_uwb->history.size() << std::endl;
        }
    }
...
void searchingAreaScreenShot(Object &input_object)
{
    std::ofstream sa_file(sa_file_name, std::ios::app);
    sa_file << to_string(input_object.id) << " " << to_string(input_object.now_state.x) << " " << to_string(input_object.now_state.y) << "\n";
    for(int i = 0; i < num_particle_samples; i++)
    {
        particle *p = input_object.searching_area->at(i);
        sa_file << to_string(p->x) << " " << to_string(p->y) << "\n";
    }
    sa_file << "\n";
    sa_file.close();
    std::cout << "safile output success!" << std::endl;
    std::ofstream screenshot_file(screenshot_file_name, std::ios::app);
    for(ObjectIterator it_screenshot = objects.begin(); it_screenshot!= objects.end();++it_screenshot)
    {
        for(LidarPointIterator it_ss_point = it_screenshot->points.begin();it_ss_point!=it_screenshot->points.end();++it_ss_point)
        {
            screenshot_file << to_string(it_screenshot->id) << " " << to_string(it_ss_point->x) << " " << to_string(it_ss_point->y) << "\n";
        }
    }
    screenshot_file.close();
    std::cout << "screenshot_file output success!" << std::endl;
}
...
void sequenceFusion()
{
    for (UwbNodeIterator it_uwb = uwbs.begin(); it_uwb != uwbs.end(); ++it_uwb)
    {
        ...
        for (ObjectIterator it_obj = objects.begin(); it_obj != objects.end(); ++it_obj)
        {
            ...
            double similarity =  jaccard_factor; //gaussian_similarity *
        ...
        if (matched_obj_id > 0)
        {
            ...
            for (ObjectIterator it_obj = objects.begin(); it_obj != objects.end(); ++it_obj)
            {
                //std::cout << it_obj->id <<"'s searching_area_is_created " <<  it_obj->searching_area_is_created << std:: endl;
                if (it_obj->id == matched_obj_id)
                {
                    it_uwb->matched_obj_id = it_obj->id;
                    it_uwb->trajectory.push_back(it_obj->now_state);
                    it_uwb->points = it_obj->points;
        ...
        if (matched_obj_id < 0)
        {
            ...
            for (ObjectIterator it_obj = objects.begin(); it_obj != objects.end(); ++it_obj)
            {
                ...
                if (it_obj->matched_uwb_id == it_uwb->id && it_obj->is_lost && it_obj->searching_area_is_created)
                {
                    ...
                    //we let it spread less than 50 frames
                    if(it_obj->lost_time < 60)
                    {
                        for (int i = 0; i < num_particle_samples; i++)
                        {
                            particle *p = it_obj->searching_area->at(i);
                        ...
                        //searchingAreaScreenShot(*it_obj);
...
void recordLocationResult(double &timestamp)
{
    uwb_lidar_tracking::target_info target_temp;
    std::ofstream uwb_file(uwb_file_name, std::ios::app);
    for (UwbNodeIterator it_uwb = uwbs.begin(); it_uwb != uwbs.end(); ++it_uwb)
    {
        uwb_file << std::to_string(it_uwb->id) << " " << std::to_string(timestamp) << " "
        ...
        target_temp.x = it_uwb->trajectory.back().x;
        target_temp.y = it_uwb->trajectory.back().y;
        target_temp.header.stamp = ros::Time::now();
        for (LidarPointIterator it_p = it_uwb->points.begin();it_p != it_uwb->points.end(); ++it_p)
        {
            double angle_temp = atan2(it_p->y,it_p->x);
            double range_temp = sqrt((it_p->y * it_p->y) + (it_p->x * it_p->x));
            target_temp.cluster_angles.push_back(angle_temp);
            target_temp.cluster_ranges.push_back(range_temp);
            target_info_pub.publish(target_temp);
        }
    }
    uwb_file.close();
}
...
void recordRightFeature(ObjectState &r_feature_state)
{
    if (flag_save_r_feature)
    {
        std::cout << "RightFeature Saving" << std::endl;
        //double timestamp = amcl_msg->header.stamp.toSec();
        std::ofstream r_feature_file(right_feature_file_name, std::ios::app);
        r_feature_file << std::to_string(1) << " " << std::to_string(0) << ":" << std::to_string(r_feature_state.area) << " " << std::to_string(1) << ":" << std::to_string(r_feature_state.depth) << " "
        << std::to_string(2) << ":" << std::to_string(r_feature_state.girth) << " " << std::to_string(3) << ":" << std::to_string(r_feature_state.width) << "\n";
        r_feature_file.close();
    }
}

void recordWrongFeature(ObjectState &w_feature_state)
{
    if (flag_save_r_feature)
    {
        std::cout << "RightFeature Saving" << std::endl;
        //double timestamp = amcl_msg->header.stamp.toSec();
        std::ofstream w_feature_file(wrong_feature_file_name, std::ios::app);
        w_feature_file << std::to_string(1) << " " << std::to_string(0) << ":" << std::to_string(w_feature_state.area) << " " << std::to_string(1) << ":" << std::to_string(w_feature_state.depth) << " "
        << std::to_string(2) << ":" << std::to_string(w_feature_state.girth) << " " << std::to_string(3) << ":" << std::to_string(w_feature_state.width) << "\n";
        w_feature_file.close();
    }
}
...
void callback(const sensor_msgs::LaserScan::ConstPtr &scan_msg, const nlink_parser::LinktrackNodeframe2::ConstPtr &range_msg)
{
    ...
    {
        std::cout << "timestamp : " << toString(scan_msg->header.stamp.toSec()) << std::endl;
        time_counter++;
        ...
        // we should sort the points with attribute "angle"
        //sortClusterPoints();
        double time_stamp = scan_msg->header.stamp.toSec();
        //targetLocalization(time_stamp);
        //targetLocalizationUWB(time_stamp);
        ...
        for(ObjectIterator itobj= objects.begin();itobj!=objects.end();++itobj)
        {
            itobj->getFeatures();
            itobj->getMovingStatus();
            // std::cout<< itobj->id << "'s features: area: " << toString(itobj->now_state.area) << "\t depth: " << toString(itobj->now_state.depth) 
            // << "\t girth: " << toString(itobj->now_state.girth) << "\t width: " << toString(itobj->now_state.width) << "\t match uwb id:" << toString(itobj->matched_uwb_id) << std::endl;
            // for(LidarPointIterator it_point = itobj->points.begin();it_point!=itobj->points.end();++it_point)
            // {
            //     std::cout << "x: "<<it_point->x << " y: " << it_point->y <<" angle: "<< it_point->angle <<std::endl;
            // }
            if(itobj->matched_uwb_id > 0)
            {
                recordRightFeature(itobj->now_state);
            }
            else
            {
                recordWrongFeature(itobj->now_state);
            }
        }
        std::cout << "target lost time = " << target_lost_time << "\n " << std::endl;
        time_count_frame ++;
    }
}

void setSupportVectorMachineParam()
{
    svm_param.svm_type = SVDD;              //RBF or OneClassSVM
    svm_param.kernel_type = RBF;            //RBF == gaussian kernel
    svm_param.degree = 3;
    svm_param.gamma = 0.1;       //for SVDD OCSVM
    svm_param.coef0 = 0;

    svm_param.cache_size = 40;
    svm_param.eps = 0.001;
    svm_param.C = 1;                        //for SVDD

    svm_param.nr_weight = 0;
    svm_param.weight_label = nullptr;
    svm_param.weight = nullptr;
    svm_param.nu = 0.5;                     //for OCSVM
    svm_param.p = 0.1;
    svm_param.shrinking = 0;
    svm_param.probability = 0;

    flag_param_is_set = true;
}

void* trainSupportVectorDataDescription(void* args)
{
    while(1)
    {
        sleep(5);
        std::cout<< "Support Vector Data Description Training In" << std::endl;

        //set svm param
        if(!flag_param_is_set)
        {
            //we only need to set param once (for the first time)
            setSupportVectorMachineParam();
        }

        //prepare training data from objects
        for(ObjectIterator it_obj = objects.begin();it_obj!=objects.end();++it_obj)
        {
            if(it_obj->matched_time > training_num)
            {
                std::cout<< "training " << it_obj->id << "'s features" << std::endl; 
                svm_problem prob;
                int data_size = (int)training_num;
                prob.l = data_size;
                prob.y = new double[prob.l];
                prob.x = new svm_node *[prob.l];

                svm_node *x_space = new svm_node[(feature_num + 1) * prob.l];

                int cnt = 0;
                for(int i = 0;i < training_num; i++)
                {
                    int head_num = cnt;
                    ObjectState tmp_obj = *(it_obj->trajectory.end()-i-1);
                    for (int j = 0;j < feature_num;j++) 
                    {
                        ...
                        {
                        case 0:
                            x_space[cnt].value = tmp_obj.area;
                            break;
                        case 1:
                            x_space[cnt].value = tmp_obj.depth;
                            break;
                        case 2:
                            x_space[cnt].value = tmp_obj.girth;
                            break;
                        case 3:
                            x_space[cnt].value = tmp_obj.width;
                            break;
                    ...
                    x_space[cnt].index = -1;
                    cnt++;
                    prob.x[i] = &x_space[head_num];
                    prob.y[i] = 1;
                }
                model = svm_train(&prob,&svm_param);
                std::cout << "svm model training success!" << std::endl;
                svm_save_model(right_feature_model_name,model);
                flag_is_svm_model_created = true;
            }
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    target_info_pub = nh.advertise<uwb_lidar_tracking::target_info>("/target_info", 1);

    //pthread_t tid;
    //int ret = pthread_create(&tid, NULL, trainSupportVectorDataDescription, NULL);
