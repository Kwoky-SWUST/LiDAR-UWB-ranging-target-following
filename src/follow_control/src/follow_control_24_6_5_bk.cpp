...
std::vector<Obstacle> obstacles_raw_global;
std::vector<Obstacle> obstacles_global;
boost::circular_buffer<std::vector<Obstacle>> obstacles_global_buffer(5);
...
double getDist(RobotState state, std::vector<Obstacle> obstacle)
{
  ...
  for (obstacle_it it_obs = obstacles_global.begin(); it_obs != obstacles_global.end(); it_obs++)
  {
...
{
  double v_low = v_min;
  double v_high = sqrt(2 * getDist(state, obstacle) * a_vmax);
  double w_low = w_min;
  double w_high = w_max;
  return {v_low, v_high, w_low, w_high};
}
...
void followerOdomCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
  ...
  if(fabs(ros::Time::now().toSec() - last_target_time.toSec()) > 2)
  {
  ...
  // if (!goal_global.status || obstacles_global_buffer.empty())
  // {
  //   control_msg.linear.x = 0;
  //   control_msg.angular.z = 0;
  //   follow_control_pub.publish(control_msg);
  ...
  double linear_speed_gain = linear_dist / 0.7;
  ...
  // {
  //   //临时加速
  //   dynamic_window[1] = dynamic_window[1] + 0.2;
  //   dynamic_window[2] = dynamic_window[2] - 0.1;
  //   dynamic_window[3] = dynamic_window[3] + 0.1;
  // }
...
{
  std::vector<Obstacle> obstacle_v_temp;
  obstacle_v_temp.clear();
  const double LIDAR_ERR = 0.05;
  const double LIDAR_MAX = 5;
  for (int i = 0; i < msg->ranges.size(); i++)
  {
    ...
    range = msg->ranges[i];
    if (i == 0 || fabs(msg->ranges[i] - msg->ranges[i - 1]) > 0.1 || i == msg->ranges.size() - 1 || fabs(msg->ranges[i] - msg->ranges[i + 1]) > 0.1)
    {
      obstacle_v_temp.push_back(Obstacle(range * cos(angle), range * sin(angle), range, angle));
    }
  }

  obstacles_raw_global = obstacle_v_temp;
  obstacle_v_temp.clear();
  ...
  std::vector<Obstacle> ob_temp = obstacles_global_buffer.back();

  new_scan.ranges.clear();

  for (int i = 0; i < msg->ranges.size(); i++)
  {
    double angle_min_diff = 1000;
    double range_diff = 100;
    double r_temp = 1000;
    for (obstacle_it it_obs = ob_temp.begin(); it_obs != ob_temp.end(); it_obs++)
    {
      double angle_temp = fabs(it_obs->a - msg->angle_min - msg->angle_increment * i);

      if (angle_min_diff > angle_temp)
      {
        range_diff = it_obs->r - msg->ranges[i];
        r_temp = it_obs->r;
        angle_min_diff = angle_temp;
      }
    }
    if (angle_min_diff < 0.001)
    {
      new_scan.ranges.push_back(r_temp);
    }
    else
    {
      new_scan.ranges.push_back(1000.0);
    }
  }
  fake_laser_pub.publish(new_scan);
}

void targetInfoCallback(const uwb_lidar_tracking::target_info::ConstPtr &msg)
{
  last_target_time = ros::Time::now();

  int size = msg->cluster_angles.size();

  if (obstacles_raw_global.empty() || size == 0)
    return;

  obstacles_global.clear();
  goal_global.x = msg->x;
  goal_global.y = msg->y;
  goal_global.status = true;

  // 把目标从障碍物中删掉 防止错误计算轨迹
  for (obstacle_it it_obs = obstacles_raw_global.begin(); it_obs != obstacles_raw_global.end(); it_obs++)
  {
    for (int i = 0; i < size; i++)
    {
      if (fabs(it_obs->a - msg->cluster_angles[i]) < 0.1 && fabs(it_obs->r - msg->cluster_ranges[i]) < 0.15)
      {
        it_obs = obstacles_raw_global.erase(it_obs);
        it_obs--;
      }
    }
  }

  obstacles_global = obstacles_raw_global;
  obstacles_global_buffer.push_back(obstacles_global);
  obstacles_global.clear();
  obstacles_raw_global.clear();
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub_follow_goal = nh.subscribe("/target_info", 1000, targetInfoCallback);
