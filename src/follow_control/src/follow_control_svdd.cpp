...
const std::string configure_file("/home/gglin/WORKSPACE/uwb_lidar_following_gglin/src/follow_control/config/dwa_parameters.txt");
...
void dynamicWindowApproach(std::vector<Obstacle> temp_obs, const Goal goal)
{
  ...
  double linear_speed_gain = linear_dist / 1.5; // TODO add this "1.5" into configure
  ...
  while (v <= dynamic_window[1] + 0.01)
  {
    ...
    while (w <= dynamic_window[3] + 0.01)
    {
      ...
      double visi_eval = 1;
      if (dist_robot_nearest_object < 1.5 && linear_dist < 1.5)
      {
...
void targetInfoCallback(const uwb_lidar_tracking::Targets::ConstPtr &msg)
{
  ...
  for (size_t i = 0; i < msg->targets.size(); ++i)
  {
    ...
    {
      target_goal.status = msg->targets[i].status; // seems useless
...
int main(int argc, char **argv)
{
  ...
  follow_control_pub = nh.advertise<geometry_msgs::Twist>("/robot1/cmd_vel_mux/input/teleop", 1);

  ros::Subscriber sub_follow_scan = nh.subscribe("/robot1/scan", 1000, followerScanCallback);
